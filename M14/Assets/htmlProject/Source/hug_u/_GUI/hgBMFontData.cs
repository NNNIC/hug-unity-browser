...
[System.Serializable]
public class hgBMFontData {

    string m_bmFontTxt; 

    int m_cutUpperSize = 0;
    int m_curLowerSize = 0;

    public hgBMFontData(string text, int upper, int lower)
    {
        m_bmFontTxt = text;
        m_cutUpperSize = upper;
        m_curLowerSize = lower;
    }
    ...
    public void Parse()
    {
        ...
        int charcnt = 0;
        int kerncnt = 0;
        var rs = new ReadText(m_bmFontTxt);
        while (true)
        {
            var line = rs.GetLine();
