...
public class hglRender {
    ...
    {
        protected hglRender        htmlRender;
        public    ALIGN             align;
        public    VALIGN            valign;
        public    List<BASE>        list;
        protected INLINE            _curLine;

        public BLOCK(hglRender hr) {htmlRender = hr; _curLine=null;}
    ...
    {
        public TABLE(hglRender render) : base(render) {
            trlist = new List<TR>();
        }
        public float vspace;
        public float hspace;
        ...
        public class TR : BLOCK
        {
            ...
            public TR(hglRender render) : base(render) {
                tdlist = new List<TD>();
            }
        ...
        public override bool Format_2(bool ShrinkWidth /*= false*/, hgRect curRect/* = null*/)
        {
            foreach (var tr in trlist)
            {
                ...
                {
                    if (td.block.Format_2(true,null))
                    { 
    ...
    public void Format(hgMesh.MeshSet ms, GameObject go )
    {
        m_root.Format_1();
        m_root.Format_2(false,null);  
        m_root.Format_3();
        
        if (m_renderInfo.m_1bone)
        {
            m_root.FORMAT_1BONE_FINAL(ms,go.transform);
        }
        else
        {
		    m_root.Format_MBONE();
            go.transform.localPosition = m_renderInfo.m_renderTopLeft;
            m_root.Format_MBONE_FINAL(m_renderInfo.m_renderTopLeft,ms,go.transform); //Debug.Log(">>>>>>>>>>>>>> " + m_renderInfo.m_renderTopLeft);
            //m_root.Format_MBONE_FINAL(new Vector2(-640,0),ms,transform);
        }
    }

    public void OutputRendering(GameObject go)
    {
        hgMesh.MeshSet ms = new hgMesh.MeshSet();
    
        Format(ms,go);

        var render = go.GetComponent<SkinnedMeshRenderer>();
        ...
        go.renderer.material.SetTexture("_PalTex",m_htmlColor.GetTexture());
    ...
    //    m_htmlColor = new hglHtmlColor();
    //    m_anchor    = new hglAnchor();
    //}
